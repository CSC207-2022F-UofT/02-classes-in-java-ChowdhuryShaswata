...
public class Silly implements Comparable<Silly>{
    ...
    /**
     * Creates a new Silly object, taking in two strings as arguments.
     *
     * @param s1 the first string component of the name
     * @param s2 the second string component of the name
     */
    ...
    public int compareTo(Silly other) {
         ...
         *                .length() method.
         */
        int lngth = this.name.length();
        int lngthother = other.name.length();
        if (lngth == lngthother){
            return 0;
        }
        if (lngth > lngthother){
